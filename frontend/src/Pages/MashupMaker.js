...
const MashupMaker = ({socket}) => {
  ...
  useEffect(() => {
    ...
    async function fetchData(){
      try {
        ...
        const contestsList= await axios.get('https://codebyte-if2s.onrender.com/api/contest/pastContests',{
  ...
  const createContest= async(e)=>{
    ...
    try {
      const {data} = await axios.post('https://codebyte-if2s.onrender.com/api/contest/createContest', formData, {
        withCredentials: true,
      });
